        ...
        <div
            ...
            <div class="social-text text-center">
                <form method="POST" action="{{ route('password.update') }}">
                    ...
                    <!-- EMAIL INPUT BOX -->
                    <div class="mt-4">
                        <label for="email" value="old('email', $request->email)">Email:</label>
                        <input id="email" class="block mnt-1 w-full placeholder-white"  type="email" name="email" placeholder="Email*" required autofocus />
                    <div>
                    <!-- PASSWORD INPUT BOX -->
                    <div class="mt-4">
                        <label for="password" value="{{__('Password') }}">Password:</label>
                        <input id="password" class="block mnt-1 w-full placeholder-white" type="password" name="password" placeholder="New Password*" required autocomplete="new-password" />
                    </div>
                    <!-- PASSWORD CONFIRMATION BOX -->
                    <div class="mt-4">
                        <label for="password_confirmation" value="{{__('Confirm Password') }}">Confirm Password:</label>
                        <input id="password_confirmation" class="block mnt-1 w-full placeholder-white" type="password" name="password_confirmation" placeholder="Confirm Password*" required autocomplete="new-password" />
                    </div>
                        <button dusk="reset-button" class="bg-gray-900 hover:bg-gray-800 text-gray-400 font-bold py-2 px-4 mt-4 rounded-full">
                            Submit
                        </button>
